...
def _auto_write_delegation_metric(
    *,
    ...
    writer = Path(__file__).resolve().parent / "write_delegation_metric.py"
...
def _resolve_script_dir(tool_file: Path) -> Path:
    """Resolve the directory containing the runtime scripts.

    The scripts live alongside this file (same directory), whether in the
    source tree (codex-job/scripts/) or after deploy (~/.claude/skills/codex-job/scripts/).
    """
...
def codex_task(
    ...
    script_dir = _resolve_script_dir(tool_file)
