...
usage() {
  cat <<'USAGE'
Usage:
  run_gemini_task.sh --repo <path> [--task <text>] [--resume <session>] [options] [-- <extra gemini args...>]

Options:
  --repo <path>         Repository/workdir for gemini --cd (required)
  --task <text>         Task prompt for Gemini (required unless --resume is used)
  --resume <session>    Resume an existing Gemini session by ID or "latest" (optional)
  --gemini-bin <path>   Gemini binary/command (default: gemini)
...
Examples:
  scripts/run_gemini_task.sh --repo /path/to/your/repo --task "Fix failing tests"
  scripts/run_gemini_task.sh --repo . --task "Implement feature" -- --model gemini-2.0-pro-exp
USAGE
}

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PARSER="$SCRIPT_DIR/parse_gemini_run.py"
...
LOG_DIR="./runs"
JSON_OUT=""
LOG_VERBOSITY="${GEMINI_LOG_VERBOSITY:-low}"
NOTIFY_CMD=""
EVENT_STREAM=""
EXTRA_ARGS=()
...
if ! GEMINI_BIN_PATH="$(command -v "$GEMINI_BIN" 2>/dev/null)"; then
  echo "Error: gemini command not found: $GEMINI_BIN" >&2
  exit 127
fi

mkdir -p "$LOG_DIR"
RUN_ID="$(date +%Y%m%d-%H%M%S)-$$"
LOG_FILE="$LOG_DIR/gemini-run-$RUN_ID.log"
META_FILE="$LOG_DIR/gemini-run-$RUN_ID.meta.json"
if [[ -n "$JSON_OUT" ]]; then
  SUMMARY_PENDING="$JSON_OUT"
else
  SUMMARY_PENDING="$LOG_DIR/gemini-run-$RUN_ID.summary.json"
fi
if [[ -n "$EVENT_STREAM" ]]; then
  mkdir -p "$(dirname "$EVENT_STREAM")"
fi
...
# Gemini CLI does not support a --cd flag; we cd into the repo before exec.
# Uses --resume for session index/id and --prompt for the task.
GEMINI_CMD=("$GEMINI_BIN_PATH")
if [[ -n "$RESUME_SESSION" ]]; then
  GEMINI_CMD+=(--resume "$RESUME_SESSION")
fi
if [[ -n "$TASK" ]]; then
  GEMINI_CMD+=(--prompt "$TASK")
fi
GEMINI_CMD+=("${EXTRA_ARGS[@]}")

SCRIPT_CMD_QUOTED="$(shell_join "${SCRIPT_CMD[@]}")"
GEMINI_CMD_QUOTED="$(shell_join "${GEMINI_CMD[@]}")"
...
if [[ "$LOG_VERBOSITY" == "high" || "$LOG_VERBOSITY" == "extreme" ]]; then
  {
    echo "===== run_gemini_task.sh debug preamble ====="
    ...
    echo "gemini_bin_input=$GEMINI_BIN"
    echo "gemini_bin_resolved=$GEMINI_BIN_PATH"
    echo "gemini_command=$GEMINI_CMD_QUOTED"
    ...
    echo "PATH=$PATH"
    if "$GEMINI_BIN_PATH" --version >/dev/null 2>&1; then
      echo "gemini_version=$("$GEMINI_BIN_PATH" --version | head -n1)"
    else
      echo "gemini_version=<unavailable>"
    fi
    echo "===== end preamble ====="
  } | tee -a "$LOG_FILE"
...
set +e
(cd "$REPO" && "${GEMINI_CMD[@]}") 2>&1 | tee -a "$LOG_FILE"
...
# Extract session ID (if available in logs)
# Gemini CLI might not emit a explicit "Session ID: XXX" like Codex
# but we can try to find it if it does.
set +e
SESSION_ID=$(
...
obj = {
    ...
    "gemini_bin": os.environ["GEMINI_BIN_ENV"],
    ...
    "exit_code": int(os.environ["GEMINI_EXIT_ENV"]),
}
...
if [[ -n "$JSON_OUT" ]]; then
  ...
  SUMMARY_PATH="$LOG_DIR/gemini-run-$RUN_ID.summary.json"
...
if [[ "$LOG_VERBOSITY" == "high" || "$LOG_VERBOSITY" == "extreme" ]]; then
  ...
  SUMMARY_LINES+=("gemini_command=$GEMINI_CMD_QUOTED")
fi
...
if [[ "$GEMINI_EXIT" -eq 0 ]]; then
  emit_event "$(make_event_json "run_completed" "success" "$GEMINI_EXIT")"
else
  emit_event "$(make_event_json "run_completed" "failure" "$GEMINI_EXIT")"
fi
