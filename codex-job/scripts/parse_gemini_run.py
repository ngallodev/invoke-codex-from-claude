...
def extract_token_usage(log_text: str) -> dict[str, Any]:
    # Gemini CLI tokens patterns (similar to Codex but might vary)
    patterns = {
        "input_tokens": [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Parse Gemini run output into normalized JSON summary.")
    parser.add_argument("--log", required=True, help="Path to raw gemini log file")
    parser.add_argument("--meta", help="Path to meta JSON emitted by run_gemini_task.sh")
