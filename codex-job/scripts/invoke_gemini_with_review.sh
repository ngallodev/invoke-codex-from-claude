...
# Enhanced Gemini invocation with automatic review on failure
# Usage: invoke_gemini_with_review.sh --repo <path> --task <text> [options]

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
RUNNER="$SCRIPT_DIR/run_gemini_task.sh"
...
obj = {
    ...
    "source": "invoke_gemini_with_review.sh",
...
# Extract metadata for this exact run
...
LATEST_RUN_ID="$(extract_kv "$RUN_OUTPUT" "gemini_run_id")"
SESSION_ID="$(extract_kv "$RUN_OUTPUT" "gemini_session_id")"
...
# Gemini CLI might not always have a long-lived session ID in the same format
# but if it's "unknown", we might still try to resume with "latest" if needed.
# However, for a robust review, we'd want a real session ID.
...
if [[ "$EXIT_CODE" -ne 0 ]]; then
  ...
  # Check if Gemini actually completed work despite exit code
  CODEX_COMPLETED=0
  REAL_FAILURE=0

  if [[ -f "$LATEST_LOG" ]]; then
    # Gemini CLI completion indicators
    if grep -qE "tokens used|prompt_tokens|completion_tokens" "$LATEST_LOG" 2>/dev/null; then
      CODEX_COMPLETED=1
      echo "✓ Gemini completed work (token usage found in log)"
    fi

    # Check for real failure indicators
    if grep -qE "error: |fatal: |Exception: |Error authenticating" "$LATEST_LOG" 2>/dev/null; then
      REAL_FAILURE=1
    fi
  fi
  ...
  if [[ -n "$GEMINI_BIN" ]]; then
    REVIEW_ARGS+=(--gemini-bin "$GEMINI_BIN")
  fi
  if [[ -n "$LOG_DIR" ]]; then
  ...
  emit_wrapper_event "review_completed" "$([[ "$REVIEW_EXIT" -eq 0 ]] && echo success || echo failure)" \
    "$(extract_kv "$REVIEW_OUTPUT" "gemini_run_id")" \
    "$(extract_kv "$REVIEW_OUTPUT" "gemini_session_id")" \
